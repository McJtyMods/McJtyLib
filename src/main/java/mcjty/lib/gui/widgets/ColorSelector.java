...
public class ColorSelector extends AbstractLabel<ColorSelector> {
    ...
    public void draw(Screen gui, GuiGraphics graphics, int x, int y) {
        ...
        if (isEnabled()) {
            ...
            drawStyledBoxNormal(window, graphics, xx, yy, xx + bounds.width - 1, yy + bounds.height - 1, 0xff000000 | color);
    ...
    private void createColorSelectorWindow(Window window, int x, int y) {
        ...
        ImageLabel colors = new ImageLabel()
                .image(ResourceLocation.fromNamespaceAndPath(McJtyLib.MODID, "textures/gui/colorpicker.png"), 0, 0)
